    ...
    def encode(self, obj):
        ...
        for match in self.regex.finditer(json_repr):
            ...
            json_repr = json_repr.replace(
                f'"{format_spec.format(id)}"', json_obj_repr
            )
...
class ABCSampler(object):
    ...
    def abc_smc(self, num_particles=500, num_iterations=10, final_epsilon=0.05):
        """
        Sequential Monte Carlo ABC implementation for patient trajectory model.
        
        Parameters:
        -----------
        num_particles : int
            Number of particles (parameter sets) to maintain
        num_iterations : int
            Number of SMC iterations (decreasing epsilon thresholds)
        final_epsilon : float
            Target final acceptance threshold
        """
        ...
        for i in tqdm.tqdm(range(num_particles), desc="Initial sampling"):
            theta = self.select_theta_init_from_prior(self.abc_prior)
            particles.append(theta)
        
        # Storage for accepted particles at each iteration
        all_particles = []
        all_distances = []
        
        # Start with a large epsilon and decrease it over iterations
        epsilon_schedule = np.logspace(np.log10(self.epsilon), np.log10(final_epsilon), num_iterations)
        
        # Main SMC loop with tqdm
        for t in tqdm.tqdm(range(num_iterations), desc="SMC Iterations"):
            #print(f"SMC Iteration {t+1}/{num_iterations}, epsilon = {epsilon_schedule[t]:.5f}")
            
            # Calculate distances for all particles with progress bar
            for i in range(num_particles):
                T_x, weights_sim, _ = self.simulate_dataset(particles[i])
                distances[i] = self.calc_distance(T_x, weights_sim)
            
            # Accept particles below epsilon threshold
            accepted_indices = np.where(distances <= epsilon_schedule[t])[0]
            acceptance_rate = len(accepted_indices) / num_particles
            accepted_particles = [particles[i] for i in accepted_indices]
            
            #print(f"Acceptance rate: {acceptance_rate:.1%} ({len(accepted_indices)}/{num_particles})")
            
            if len(accepted_indices) > 0:
                accepted_weights = weights[accepted_indices] / np.sum(weights[accepted_indices])
            else:
                # Handle case with no acceptances
                #print("No particles accepted. Adjusting epsilon...")
                # Accept top 5% of particles
                sorted_indices = np.argsort(distances)
                cutoff = max(int(num_particles * 0.05), 1)
                accepted_indices = sorted_indices[:cutoff]
                accepted_particles = [particles[i] for i in accepted_indices]
                accepted_weights = np.ones(len(accepted_indices)) / len(accepted_indices)
            
            # Store accepted particles
            all_particles.append(accepted_particles)
            all_distances.append(distances[accepted_indices])
            
            # Prepare for next iteration - resample particles
            if t < num_iterations - 1:
                ...
                particles_resampled = [accepted_particles[i] for i in resampled_indices]
                
                # Perturb particles through MCMC kernel
                particles = []
                for i in range(num_particles):
                    perturbed = self.perturb_parameters(particles_resampled[i], scale_factor=0.8/(t+1.5))
                    particles.append(perturbed)
                
                # Reset weights
                weights = np.ones(num_particles) / num_particles
    
        return all_particles, all_distances
    ...
    def perturb_parameters(self, theta, scale_factor=0.7):
        """
        Perturb parameters for SMC ABC using adaptive proposal distributions
        """
        theta_prime = deepcopy(theta)
        
        # Perturb health state probabilities
        for s in range(len(self.config_dict['states'])):
            theta_prime['health'][s] = self.draw_proposal_distribution_categorical(
                theta['health'][s], 
                scale=100 * scale_factor
            )
            
            if self.state_name_id_map[s] != "OnVentInICU":
                theta_prime['die_after_declining'][s] = self.draw_proposal_distribution_categorical(
                    theta['die_after_declining'][s], 
                    scale=200 * scale_factor
                )
        ...
        for health_state in ["Declining", "Recovering"]:
            for s in range(len(self.config_dict['states'])):
                # Perturb lambda
                theta_prime['durations'][health_state][s]['lam'] = self.draw_proposal_distribution_truncatednormal(
                    theta['durations'][health_state][s]['lam'],
                    0.5 * scale_factor,
                    theta['durations'][health_state][s]['lower'],
                    theta['durations'][health_state][s]['upper']
                )
                
                # Perturb tau
                theta_prime['durations'][health_state][s]['tau'] = self.draw_proposal_distribution_normal(
                    theta['durations'][health_state][s]['tau'],
                    0.1 * scale_factor
                )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_samples_to_save', default=100, type=int)  # Default to 100 samples
    ...
    parser.add_argument('--start_epsilon', default=1.0, type=float) # Heuristically reasonable starting point. Theoretical upper bound is 1.
    ...
    parser.add_argument('--algorithm', default='abc', choices=['abc', 'smc'], 
                        help='Sampling algorithm: abc or smc')
    ...
    if algorithm == 'abc':
        ...
        all_particles, all_distances = sampler.abc_smc(num_particles=500, num_iterations=num_iterations, final_epsilon=0.05)
    end = time.time()
    ...
    if algorithm == 'abc':
        ...
        # Save all_particles to a JSON file
        particles_file = os.path.join(args.output_dir, 'smc_particles.json')
        with open(particles_file, 'w+') as f:
            json.dump(all_particles, f, cls=MyEncoder, indent=2)

        # Save multiple particles from the last iteration
        num_samples_to_save = min(len(all_particles[-1]), int(args.num_samples_to_save))  # Ensure it's an integer
        sorted_indices = np.argsort(all_distances[-1])  # Sort by distance
        representative_particles = [all_particles[-1][i] for i in sorted_indices[:num_samples_to_save]]  # Save the first N particles
        parameters_file = os.path.join(args.output_dir, 'smc_parameters.json')
        sampler.save_thetas_to_json(representative_particles, parameters_file)

        # Update the config to point to the parameters file
        config_copy['parameters'] = parameters_file  # Use the file path, not the dictionary

        # Save the updated config_copy to the config_post_output file
        with open(config_post_output, 'w+') as f:
            f.write(json.dumps(config_copy, cls=MyEncoder, indent=2))

        # Save distances to a CSV file
        stats = {'all_distances': all_distances}
        save_stats_to_csv(stats, stats_output)
